  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden", background: "#f2f2f7" }}>
      <div style={{ flex: 1, overflow: "hidden", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ display: tab === "learn"   ? "flex" : "none", flexDirection: "column", height: "100%", overflow: "hidden" }}><LearnPage initialStage={learnStage} onBadge={handleBadge} onComplete={handleComplete} onGoToCode={() => setTab("code")} /></div>
      ...
      {toast && (
        <div style={{
          ...
          <div style={{ fontSize: 20, fontWeight: 900, letterSpacing: -0.3, color: "#000" }}>{toast}</div>
          <div style={{ fontSize: 14, color: "#8e8e93", fontWeight: 500 }}>뱃지 획득!</div>
        </div>
      )}
    </div>
  );
